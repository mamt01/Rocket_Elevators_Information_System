...
def authenticate
  ...
  def index
      ...
      @result  = conn.exec('SELECT extract(YEAR FROM creation_date) AS YEAR, extract(to_char(MONTH FROM creation_date)) AS MONTH, COUNT(quote_id)
      FROM      factquotes
      GROUP BY  YEAR, MONTH
      ORDER BY  YEAR, MONTH')
      ...
      @result1 = conn.exec('SELECT extract(YEAR FROM date_of_creation) AS YEAR, extract(to_char(MONTH FROM date_of_creation)) AS MONTH, COUNT(contact_id)
      FROM      factcontact
      GROUP BY  YEAR, MONTH
      ORDER BY  YEAR, MONTH')
